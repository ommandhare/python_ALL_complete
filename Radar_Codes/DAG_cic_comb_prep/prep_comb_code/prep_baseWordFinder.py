...
def synonymFinderPrg(**kwarg):

    # ------------------------- bigcloud setup ------------------------------#
    #os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = cfg.service_account_key
    client = bigquery.Client()
    # ------------------------- input table --------------------------------#
    wordCntTable_Q = cfg.prepWordCntTableQuery.replace('PROJECTID',cfg.projectId).replace('DATASETID',cfg.dataSetId)\
        .replace('TABLEID',cfg.prepWordCntId).replace("{{ params.env }}",kwarg['params']["env"])
    synonymTable = f'{cfg.projectId}.{cfg.dataSetId}.{cfg.prepSynonymId}'.replace("{{ params.env }}",kwarg['params']["env"])
    ...
    # ------------------------ upload data to gcp ------------------------------- #
    job = client.load_table_from_dataframe(df,synonymTable, job_config=jobConf)
    print(f"step 2 : prep synonym   {job.result().state}")
