...
def compReplacing(compDict, snt):
    sntLst = re.findall(r'[A-Za-z]+', snt)
    currDict = {}
    ...
    for cmbStr, (size, cSize) in currDict.items():
        ...
        # ✅ remove words using REGEX (safe)
        for wd in cmbLst:
            tmpStr = re.sub(rf'\b{wd}\b', '', tmpStr)

        # ✅ clean extra spaces
        tmpStr = re.sub(r'\s+', ' ', tmpStr).strip()

        # ✅ append composite string
        tmpStr += ' ' + cmbStr
