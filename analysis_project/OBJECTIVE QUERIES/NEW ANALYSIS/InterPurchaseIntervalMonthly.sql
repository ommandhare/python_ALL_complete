 ...
 -- INTER_PURCHASE_INTERVAL AVERAGE MONTHLY
 
 
 select member_id,yearly,monthly,round(avg(inter_purchase_interval))  as average_IPI
 from
 (
 select *,date_now-last_date as inter_purchase_interval from 
    ...
    group by member_id,date_now,yearly,monthly
  )s2
  group by member_id,yearly,monthly